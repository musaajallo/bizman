...
import { getProjectsForClient } from "@/lib/actions/projects";
...
export default async function ClientProjectsPage({
  ...
  const projects = await getProjectsForClient(tenant.id);

  return (
    <div>
      <TopBar
        title="Projects"
        subtitle={`Projects for ${tenant.name}`}
        actions={
          <div className="flex items-center gap-2">
            <ViewSwitcher currentView="list" baseUrl={`/clients/${slug}/projects`} />
            <Link href={`/clients/${slug}/projects/new`}>
        ...
        {projects.length === 0 ? (
          <Card>
            <CardContent className="pt-6 flex flex-col items-center justify-center text-center py-12">
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-4">
                Create a project for {tenant.name}.
              </p>
              <Link href={`/clients/${slug}/projects/new`}>
                <Button size="sm" className="gap-2">
                  <Plus className="h-3.5 w-3.5" />
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map((project) => (
              <Link
                key={project.slug}
                href={`/clients/${slug}/projects/${project.slug}`}
              >
