...
import { getProjectsForClient, updateProjectStatus } from "@/lib/actions/projects";
...
type Project = Awaited<ReturnType<typeof getProjectsForClient>>[number];

export default function ClientProjectsBoardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const p = await getProjectsForClient(tenant.id);
    setProjects(p);
    setLoading(false);
  }, [params.slug]);
  ...
  const baseUrl = `/clients/${params.slug}/projects`;

  if (loading) {
    return (
      <div>
        <TopBar title="Projects" subtitle="" />
        <div className="p-6">
  ...
  return (
    <div>
      <TopBar
        title="Projects"
        subtitle={`Projects for ${tenantName}`}
        actions={
          <div className="flex items-center gap-2">
            <ViewSwitcher currentView="board" baseUrl={baseUrl} />
        ...
        {projects.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mt-1 mb-4">
              Create a project for {tenantName}.
            </p>
            <Link href={`${baseUrl}/new`}>
              <Button size="sm" className="gap-2">
                <Plus className="h-3.5 w-3.5" />
